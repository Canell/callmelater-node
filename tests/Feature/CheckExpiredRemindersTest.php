...
class CheckExpiredRemindersTest extends TestCase
{
    ...
    public function test_does_not_expire_executed_reminder(): void
    {
    ...
    public function test_does_not_expire_http_actions(): void
    {
        $action = ScheduledAction::create([
            'account_id' => $this->user->account_id,
            'created_by_user_id' => $this->user->id,
    ...
    public function test_expires_multiple_reminders(): void
    {
        $action1 = $this->createAwaitingReminder(['token_expires_at' => now()->subHour()]);
        $action2 = $this->createAwaitingReminder(['token_expires_at' => now()->subMinutes(30)]);
        $action3 = $this->createAwaitingReminder(['token_expires_at' => now()->addHour()]); // Not expired
    ...
    {
        $action = $this->createAwaitingReminder(['token_expires_at' => now()->addHour()]);
