    ...
    private function isValidRecipient(string $recipient, ?string $accountId): bool
    {
        ...
        if (str_contains($recipient, ':')) {
            ...
            if ($scheme === 'email' && isset($parts[1])) {
                $email = implode(':', array_slice($parts, 1)); // Handle emails with colons (rare but possible)
            ...
            if ($scheme === 'contact' && isset($parts[1])) {
                $contactId = $parts[1];
                if ($this->isUuid($contactId) && $accountId) {
            ...
            if ($scheme === 'channel') {
                // Channel recipients are handled separately, skip validation here
                return true;
            }
        }
